...
- **Aloha-Sixty Design-System** — Deployed 2026-04-24. App-weiter Theme-Wechsel vom Mint/Amber-shadcn-Default auf die Vintage-Hawaii-Briefmarken-Palette (Terracotta `#C94A2B` primary, Mustard `#E9B63A` accent, Teal `#2A6A6A` secondary, Forest `#1E4A3C`, Cream `#F2E7CE` background). Fonts: Alfa Slab One (display), Oswald (headline, ungenutzt noch), Work Sans (body) — Caveat + Dancing_Script entfernt. Alle 20 `font-[family-name:var(--font-caveat)]`-Occurrences auf `font-display` migriert. Neue `.paper-texture`-Utility (radial-gradient Dot-Pattern) für Hero-Bereiche. `.dark`-Variante aus `globals.css` entfernt (war Dead Code). **Hinweis:** PDF-Rendering (`@react-pdf/renderer` in `src/components/pdf/*`) nutzt weiterhin Caveat-TTF aus `/public/fonts/` — bewusst nicht mit-migriert, PDFs funktionieren unverändert; Konsolidierung bei Bedarf in einem Folge-Pass. Handoff-Bundle kam aus claude.ai/design.
